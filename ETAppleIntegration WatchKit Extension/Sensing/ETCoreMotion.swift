...
class ETCoreMotion {
    ...
    private func accelerometerHandler(accelerometerData:CMAccelerometerData?, error: Error?) {
        if error != nil {
            ...
            guard let acquisitionFile = ETAcquisitionFile.getInstance() else { return }
            
            let dataSourceId = ACCELEROMETER_DATA_SOURCE_ID
            let timestamp = Int64(Date(timeInterval: accelerometerData.timestamp, since: ETTools.bootTime).timeIntervalSince1970 * 1000)
    ...
    private func deviceMotionHandler(deviceMotion:CMDeviceMotion?, error: Error?) {
        if error != nil {
            ...
            let timestamp = Int64(Date(timeInterval: deviceMotion.timestamp, since: ETTools.bootTime).timeIntervalSince1970 * 1000)
