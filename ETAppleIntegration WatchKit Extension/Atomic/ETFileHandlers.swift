...
class ETAcquisitionFile {
    ...
    // singleton
    private static var instance: ETAcquisitionFile?
    static func getInstance() -> ETAcquisitionFile? {
        if instance == nil {
            do {
                instance = try ETAcquisitionFile(fileName: ACQUISITION_FILENAME)
            } catch {
    ...
    func moveContentToBatchFiles() throws {
        try queue.sync {
            ...
            let batches = ETTools.splitToBatches(array: lines, batchSize: BATCH_SIZE)
...
class ETSubmissionFile {
    private static var instances: [String:ETSubmissionFile] = [:]
    private let documentsUrl: URL
    ...
    static func getInstance(fileName: String) -> ETSubmissionFile? {
        if let instance = instances[fileName] {
            return instance
        } else {
            do {
                let newInstance = try ETSubmissionFile(fileName: fileName)
    ...
    func readBatchContent() throws -> ([Int32], [Int64], [Data]) {
    ...
    func removeFile() throws {
        ...
        ETSubmissionFile.instances.removeValue(forKey: fileName)
    }
